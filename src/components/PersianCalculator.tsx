...
const PersianCalculator: React.FC = () => {
  ...
  const validateInputs = (): boolean => {
    ...
    // Check if previous date is newer than current date
    if (!isDateNewer(lastDate, prevDate)) {
      setError('تاریخ قاعدگی ماه قبل باید از ماه جاری قدیمی‌تر باشد. لطفاً از دکمه جابجایی تاریخ‌ها استفاده کنید.');
      return false;
    }
